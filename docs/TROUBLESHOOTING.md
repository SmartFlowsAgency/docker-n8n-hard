...
| Web UI not accessible           | Check that `nginx-rproxy` and `n8n` are healthy; verify domain and SSL.           |
...
| Changes to .env not applied     | Run `./dn8nh.sh setup` then `./dn8nh.sh deploy` after editing configuration.     |
